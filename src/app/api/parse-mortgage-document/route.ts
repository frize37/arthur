...
const TRACK_SCHEMA = {
  ...
  properties: {
    ...
    anchorBasis: {
      type: "string",
      nullable: true,
      description: "The rate basis as printed on the document, e.g. 'פריים', 'עוגן בנק ישראל', 'מק\"מ' — null for fixed tracks",
    },
...
חלץ מהמסמך:
1. לכל מסלול/הלוואה בנפרד: שם הבנק, סוג ריבית (קבועה/משתנה), בסיס הריבית כפי שכתוב במסמך (למשל "פריים", "עוגן בנק ישראל", "מק\"מ" — null אם קבועה), האם צמודה למדד, שיטת פרעון, שיעור ריבית שנתית נוכחי, ריבית עוגן, שיעור תוספת/הפחתה מעל העוגן, תאריך שינוי ריבית קרוב (אם רלוונטי — זה חשוב, אל תשמיט אם קיים), יתרת תקופה בחודשים, יתרת קרן, ריבית צבורה, יתרת פיגור, ריבית פיגורים, סה"כ יתרה לסילוק של המסלול, עמלת פרעון מוקדם של המסלול, שיעור ריבית לצרכי השוואה, שיעור ריבית כוללת חזויה.
